...
**K. Santhosh — Final-Year Engineering Student | RTL Design | Design Verification**

This portfolio contains **115 structured Verilog/SystemVerilog projects** covering RTL design, CPU architecture, SoC integration, protocols, CDC, verification, assertions, functional coverage, randomized verification, UVM-style environments, and timing analysis.

### Selected Portfolio Evidence

| Project | Focus | Final Result |
|---|---|---:|
| **115 — 5-Stage Pipelined RISC Processor** | CPU / Pipeline / Hazards | **24/24 checks passed** |
| **114 — Mini RISC Processor** | CPU / RTL | **18/18 checks passed** |
| **112 — APB Peripheral Subsystem** | SoC / Bus / Peripherals | **23/23 checks passed** |
| **113 — FIFO Data Acquisition** | RTL / FIFO / Integration | **31/31 checks passed** |
| **98 — Randomized FIFO Verification** | Randomized Verification | **3,659 checks / 0 failures** |
| **99 — Randomized UART Verification** | Protocol Verification | **1,010/1,010 tests passed** |
| **100 — Randomized APB Verification** | Bus Verification | **510/510 transactions passed** |
| **105 — UVM-Style SPI Verification** | Verification | **1,080 checks / 0 failures** |
| **110 — Timing Closure Exercise** | STA / Timing | **All closure checks passed** |

### Primary Skills

`Verilog` `SystemVerilog` `RTL Design` `Design Verification` `CPU Architecture` `SoC` `FIFO` `UART` `SPI` `I2C` `APB` `CDC` `Assertions` `Functional Coverage` `Randomized Verification` `Timing Analysis`

### Tools

`Icarus Verilog` `GTKWave` `Python` `VS Code` `PowerShell` `Git` `GitHub`

> This repository is the detailed technical portfolio. The accompanying resume presents the strongest projects in a concise, role-targeted format for RTL Design and Design Verification opportunities.
